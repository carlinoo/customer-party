...
class Customer:
    ...
    def distance_from(self, lat, lon):
        ...
        # get in radians
        lat1, long1, lat2, long2 = map(math.radians, [self.latitude, self.longitude, lat2, long2])

        total_long = long2 - long1
        total_lat = lat2 - lat1

        # We use haversine formula
        a = math.sin(total_lat/2)**2 + math.cos(lat1) * math.cos(lat2) * math.sin(total_long/2)**2
